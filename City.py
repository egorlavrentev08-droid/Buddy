# city.py - Клановый город (упрощённая версия для тестов)
# Версия: 0.1.0
...
def format_city_map(city_map):
    """Форматирует карту для отправки"""
...
async def city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    city_map = clan.get('city_map', [[CELL_EMPTY for _ in range(MAP_SIZE)] for _ in range(MAP_SIZE)])
...
async def city_build(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if building_type not in valid_buildings:
        await update.message.reply_text(f"❌ Неизвестный тип. Доступны: {', '.join(valid_buildings)}")
        return
