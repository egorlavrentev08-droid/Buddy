...
def can_build(clan, building_type, row, col):
    ...
    resources = get_resources(clan)
    city_crystals = resources.get('crystals', 0)
    old_crystals = clan.treasury_crystals
    
    if city_crystals >= price or old_crystals >= price:
        return True, price
    else:
        return False, f"❌ Не хватает кристаллов. Нужно: {price} (в казне города: {city_crystals}, в казне клана: {old_crystals})"
...
def build_building(clan, building_type, row, col):
    ...
    if city_crystals >= price:
        resources['crystals'] = city_crystals - price
        save_resources(clan, resources)
    elif old_crystals >= price:
        clan.treasury_crystals = old_crystals - price
...
async def city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если есть аргументы — разбираем подкоманду
    ...
    if subcommand == 'map':
        ...
        await update.message.reply_text("❌ Неизвестная подкоманда. Доступно: map, build, upgrade, craft, mine, raid")
